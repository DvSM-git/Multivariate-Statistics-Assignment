...
sim_step <- function(i, n_train, n_test, p, beta, plot = FALSE) {
  ...
  evaluate_models <- function(y_train, X_train) {
    # OLS (matrix interface, already correct)
    ...
    b_MM <- coef(MASS::rlm(y ~ 0 + X, psi = psi.huber, maxit = 100))
    ...
    hub_loss <- function(e, delta = 1) ifelse(abs(e) < delta, 0.5 * e^2,
                                              delta * (abs(e) - 0.5 * delta))
    huber_risk <- function(e, delta = 1) mean(hub_loss(e, delta))
    rmse     <- function(e) sqrt(mean(e^2))
    
    loss_names <- c("RMSE", "MAE", "MedAE", "Huber")
    est_names  <- names(preds)  # "OLS", "M", "MM", "lts"
...
sim_step <- function(i, n_train, n_test, p, beta, plot = FALSE) {
  ...
  evaluate_models <- function(y_train, X_train) {
    ...
    # MM-estimator with Huber psi
    b_MM <- coef(MASS::rlm(y_train ~ 0 + X_train))
    ...
    mae      <- function(e) mean(abs(e))
    medae    <- function(e) median(abs(e))
    # c=1.345 => 95% efficiency
    hub_loss <- function(e, delta = 1.345) ifelse(abs(e) < delta, 0.5 * e^2,
                                              delta * (abs(e) - 0.5 * delta))
...
# Calculate the median of each metric for each scenario
formula_str <- paste("cbind(", paste(metric_names, collapse = ", "), ") ~ Scenario")
summary_stats <- aggregate(as.formula(formula_str), data = results_df, median)
